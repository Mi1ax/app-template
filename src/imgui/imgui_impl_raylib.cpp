...
static void rlImGuiRenderTriangles(unsigned int count,
                                   ...
                                   void* texturePtr)
{
    ...
    auto texture = (Texture*)texturePtr;
    ...
    for (int i = 0; i <= (count - 3); i += 3)
    {
        ...
        auto index = static_cast<int>(indexStart) + i;
        ImDrawIdx indexA = indexBuffer[index];
        ImDrawIdx indexB = indexBuffer[index + 1];
        ImDrawIdx indexC = indexBuffer[index + 2];
...
static void EnableScissor(ImDrawData* data,
                          const ImDrawCmd* cmd)
{
    rlEnableScissorTest();
    ImGuiIO& io = ImGui::GetIO();
    auto clipOff = data->DisplayPos;
    auto clipScale = GetWindowScaleDPI();

    auto fbHeight = data->DisplaySize.y * clipScale.y;

    auto clipMin = Vector2{
            (cmd->ClipRect.x - clipOff.x) * clipScale.x,
            (cmd->ClipRect.y - clipOff.y) * clipScale.y
    };

    auto clipMax = Vector2{
...
static void rlRenderData(ImDrawData* data)
{
    ...
    for (int l = 0; l < data->CmdListsCount; ++l)
    {
        ...
        for (const auto& cmd : commandList->CmdBuffer)
        {
            ...
            rlImGuiRenderTriangles(cmd.ElemCount,
                                   cmd.IdxOffset,
...
void rlImGuiSetup()
{
    ...
    io.Fonts->AddFontFromFileTTF(
            "assets/fonts/OpenSans/static/OpenSans/OpenSans-Bold.ttf",
            18.0f);
    io.FontDefault = io.Fonts->AddFontFromFileTTF(
            "assets/fonts/OpenSans/static/OpenSans/OpenSans-Regular.ttf",
            18.0f);
...
[[maybe_unused]] void rlImGuiImageRect(const Texture* image,
                                       ...
                                       Rectangle sourceRect)
{
    ...
    {
        uv0.x = -((float)sourceRect.x / imageSize.x);
        uv1.x = (uv0.x - (float)(fabs(sourceRect.width) / imageSize.x));
    }
    else
    {
        uv0.x = (float)sourceRect.x / imageSize.x;
        uv1.x = uv0.x + (float)(sourceRect.width / imageSize.x);
    }

    if (sourceRect.height < 0)
    {
        uv0.y = -((float)sourceRect.y / imageSize.y);
        uv1.y = (uv0.y - (float)(fabs(sourceRect.height) / imageSize.y));
    }
    else
    {
        uv0.y = (float)sourceRect.y / imageSize.y;
        uv1.y = uv0.y + (float)(sourceRect.height / imageSize.y);
    }

    ImGui::Image((ImTextureID)image,
                 ImVec2(float(destWidth), float(destHeight)),
                 uv0, uv1);
}